// This example was partly stolen from the sort package.
// It shows off some pretty cool ast behavior
...
func main() {
	af, err := parser.ParseFile(fset, "inputs/first.go", nil, 0)
	if err != nil {
		log.Fatal(err)
	}

	// Change package name to out
	fmt.Println("Name =", af.Name.String())
	af.Name.Name = "out"
	...
	for _, d := range af.Decls {
		...
		fmt.Println("FuncName =", fd.Name.String())
